...
/*   Updated: 2022/11/17 15:22:03 by jjuntune         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	pattern_at(t_hit_record hit,t_point hit_loc, t_color *hit_color, t_perlin *perlin)
{
	...
	if (hit.object->material.pattern.pattern_id == GRID)
			pattern_grid(hit_loc, hit_color,hit.object->material.pattern.pattern_dif, perlin_out);
	if (hit.object->material.pattern.pattern_id == STRIPED)
	{
		if (pattern_striped(hit_loc.s_xyzw.x + perlin_out) == 1)
			*hit_color = tuple_scalar_mult(*hit_color, hit.object->material.pattern.pattern_dif);
