...
/*   Updated: 2022/12/12 14:09:01 by jsaarine         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	render_image(t_main	*main, int task, int ant_al)
{
	...
	while (y < WIN_H)
	{
		...
		while (x < WIN_W)
		{
			...
			// main->sdl.frame_buffer.data[((y * WIN_W) + x++)] = color_to_int(/* copy.ray.hit.color */ color);
			img_pixel_put(&main->sdl.frame_buffer, x, y, color);
			x++;
		}
		y += NUM_TASKS;
	}
